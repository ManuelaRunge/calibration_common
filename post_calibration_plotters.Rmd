...
all_scores %>%
  ...
  print(bp)
  ggsave(paste(paste(scenario,0,sep="-"),"simulations/output",exp,"performance/scores",paste("scores_total.png",sep="_"),sep='/'),bp,height = 8,width=8)
...
all_scores %>%
  ...
  print(bp)
  ggsave(paste(paste(scenario,0,sep="-"),"simulations/output",exp,"performance/scores",paste("scores_total_by_round.png",sep="_"),sep='/'),bp,height = 8,width=8)
...
all_scores %>%
  ...
  print(bp)
  ggsave(paste(paste(scenario,0,sep="-"),"simulations/output",exp,"performance/scores",paste("scores_objective.png",sep="_"),sep='/'),bp,height =8,width=10)
...
all_scores %>%
  ...
  print(bp)
  ggsave(paste(paste(scenario,0,sep="-"),"simulations/output",exp,"performance/scores",paste("scores_objective_by_round.png",sep="_"),sep='/'),bp,height =8,width=10)
...
library(patchwork)
  ...
  ggsave(paste(paste(scenario,0,sep="-"),"simulations/output",exp,
               "performance/scores",paste("scores_best_sets.png",sep="_"),sep='/'),bp,height =12,width=16)
...
params %>%
  ...
  pp
  ggsave(paste(paste(scenario,0,sep="-"),"simulations/output",exp,"performance/parameters","emod_values.png",sep='/'),pp,height = 12,width=10)
  ...
  labs(color=NULL) + ylab("EMOD parameter value") + xlab("Batch #") -> pp
  pp
  ggsave(paste(paste(scenario,0,sep="-"),"simulations/output",exp,"performance/parameters","unit_values.png",sep='/'),pp,height = 12,width=10)
```
...
for(s in unique(dens_df$Site)){
  ...
  if(s=="dapelogo_2007"){
     ...
     for(ds in unique(ref_dens$site)){
      ...
      ggsave(paste(paste(scenario,"-",as.numeric(ddmp$iivt[i])-1,sep=''),"simulations/output",exp,"objectives",paste(ds,"asexual_densities.png",sep="_"),sep='/'),pp,height = 8,width=8)
      ...
      scale_size_continuous(range = c(0.5,3)) +
      theme(legend.position = "none") -> pp
      ggsave(paste(paste(scenario,"-",as.numeric(ddmp$iivt[i])-1,sep=''),"simulations/output",exp,"objectives",paste(ds,"gameotcyte_densities.png",sep="_"),sep='/'),pp,height = 8,width=8)
     }
  }
  }
...
ls_list <- list.files(glue(scenario,"/simulations/output/",exp,"/performance/GP"))
...
if(length(ls_list)>1){
  ...
  for(ls in ls_list){
    if(!(exists("LS"))){
      LS <- read.csv(glue(scenario,"/simulations/output/",exp,"/performance/GP/",ls))
    } else {
      tmp <- read.csv(glue(scenario,"/simulations/output/",exp,"/performance/GP/",ls))
  ...
  ggsave(paste(scenario,"lengthscales.png",sep="_"),width=12,height=8)
  ...
  ggsave(paste(scenario,"ranked_lengthscales.png",sep="_"),width=12,height=8)
  ...
  ggsave(paste(scenario,"average_ranked_lengthscales.png",sep="_"),width=10,height=6)
